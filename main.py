...
def procesar_mientrasq(instr, ts) :
...
def procesar_instrucciones(instrucciones, ts) :
    ...
    for instr in instrucciones :
        ...
        elif isinstance(instr, Mientrasq) : procesar_mientrasq(instr, ts)
        elif isinstance(instr, Si) : procesar_si(instr, ts)
        elif isinstance(instr, SiSino) : procesar_si_sino(instr, ts)
        else : print('Error: instrucción no válida')

f = open("C:/Users/Juan/Documents/2022-1/Compiladores/Finalfinal/entrada.txt", "r")
input = f.read()
